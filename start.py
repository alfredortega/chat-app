...
start.py — Ensure the venv exists, install dependencies if needed,
           pull latest code from git, start the Flask server, and open the app URL in the default browser.
...
def ensure_git_updated():
    """Pull latest changes from git repository if available."""
    if not is_git_repo():
    ...
    try:
        ...
        print("WARNING: git command not found. Skipping git pull.")
        return

    # Try to pull updates
    try:
        print("Updating code from git repository...")
